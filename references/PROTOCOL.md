...
- Verification profile is set before implementation. Under `FULL`, ordinary functional work requires an independent Tester and Reviewer; pure documentation can omit runtime testing with a concrete reason; high-risk behavior requires both and integrated acceptance. Eligible `LIGHTWEIGHT` may preselect Tester N/A, but always requires Developer targeted verification and a fresh independent Reviewer. Do not relax required checks after failure without an authorized policy/scope decision.
...
- Use task-scoped subagents, not user-visible top-level conversations, unless the user explicitly requests new conversations. Fresh context means no full-chat fork by default. Send only role, Task path, frozen approved Spec reference for `FULL` or inline approved scope digest for `LIGHTWEIGHT`, candidate/base and necessary project paths. Reviewer receives original criteria and evidence, not instructions to agree with Developer conclusions.
- When the host permits caller-selected names, reuse the Task-specific number and purpose slug for every Worker Agent and append its accurate role: `task_<sequence>_<short_name>_<role>`. For a Task displayed as `TASK-001 — Layout adjustment`, request `task_001_layout_adjustment_developer`, `task_001_layout_adjustment_tester` and `task_001_layout_adjustment_reviewer`; a different Task uses its own dynamically chosen slug. If the host requires restricted characters, derive the lower-snake-case slug from the human-readable Task purpose. The readable name communicates Task purpose and Worker role but is not authentication, formal identity or acceptance evidence; the native identity returned by the host remains authoritative. If the host assigns names or rejects the preferred form, preserve its native identity and record the closest supported readable label instead of fabricating an identity. Never treat retrospective renaming of a current Agent as implementation or acceptance evidence.
...
Changing the main conversation does not create a new Spec or Task. Repository records and verifiable native runtime evidence remain authoritative; prompts and old chat do not. A replacement prompt starts with Project ID, absolute local repository path, GitHub/remote URL and current stable version. These fields locate the intended checkout for cross-checking. They do not replace authoritative evidence or promise path access on another host. The replacement Manager inspects the supplied checkout before reading its AGENTS and PROJECT_STATE. It does not assume the initial workspace is the project. Missing, inaccessible or conflicting identity stops takeover for Human resolution. Do not search unrelated projects, guess an identity or generate a replacement ID.
...
5. Marks coordination `READY_FOR_TAKEOVER`. Returns one copy-ready replacement prompt populated with every identity and state field from step 4. Then ceases coordination for that project scope.

The replacement Manager:

1. Inspects the supplied absolute checkout first. It does not assume the initial workspace is the project. Then it reads AGENTS, state, active Tasks, and each Task's frozen approved Spec for `FULL` or inline approved scope for `LIGHTWEIGHT` from that checkout.
2. Cross-checks all identity and state fields against repository and native evidence. A different worktree may not contain uncommitted or ignored files.
