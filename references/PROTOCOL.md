...
- Give every new Task a stable numbered ID and a concise descriptive `short_name`. The ID remains the machine-facing identity; the short name is a non-authoritative usability label shown beside the ID in the active-work index and human-facing guidance. Use one to four meaningful lower-case ASCII words separated by underscores, avoiding generic labels such as `work`, `task` or `update` when a clearer outcome is known. Do not rename historical Tasks or identities.
...
- When the host permits caller-selected names, request `task_<sequence>_<short_name>_<role>` for each new Worker Agent, for example `task_005_agent_names_developer`, `task_005_agent_names_tester` and `task_005_agent_names_reviewer`. This readable name communicates Task purpose and Worker role but is not authentication or formal identity evidence; the native identity returned by the host remains authoritative. If the host assigns names or rejects the preferred form, preserve its native identity and record the closest supported readable label instead of fabricating an identity.
...
1. Inspects the supplied absolute checkout first. It does not assume the initial workspace is the project. Then it reads AGENTS, state, active Tasks and their approved Specs from that checkout.
